...
for i=1:group
    m=i;
    for j=1:N1
    output(:,j,i)=output_ori(:,m);  %put N1 samples in each time slot
    m=m+1;
    end
end
...
for ii=1:group
    for i=1:N1
        for j=1:N1
                temp=corrcoef(real(output(:,i,ii)),real(output(:,j,ii)));
                A(i,j,ii)=temp(1,2);
                temp=corrcoef(imag(output(:,i,ii)),imag(output(:,j,ii)));  
                C(i,j,ii)=temp(1,2);            
        end
    end
end
